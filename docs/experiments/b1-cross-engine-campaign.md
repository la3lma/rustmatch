...
`d725161`. That revision includes the exact initial matrix, fresh-host resumable
...
Its `regexset-harness-v2` runner accounts for every full-corpus scan in either
the declared warm-up or measurement arrays; the earlier 20-point v1 attempt was
rejected and restarted from zero.
