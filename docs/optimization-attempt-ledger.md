...
> Generated from [`optimization-attempts.json`](optimization-attempts.json). Do not edit this file directly. Last reviewed: **2026-08-07**.
...
|---:|---|---|---|---|---|
| 1 | [**H43-X2 - Explicit isolated assertion-prefix matcher prototype**](adr/0009-explicit-experimental-matcher.md) | ADR accepted; isolated source prototype available | H43-A1 retained exact 204x-408x target throughput, while H43-A1-P1 proved the generic and specialized build entry points phase-locally neutral with exact allocation and structure equivalence. ADR-0009 now provides a separate-type boundary that can protect normal Matcher users even when Cargo features are unified. | Extract the benchmark-only mechanism behind unstable-assertion-prefix-v1 and a separate experimental matcher type. Pass local semantic, callback/failure/reuse, resource, feature-off, and feature-on/runtime-unused isolation gates before authorizing any new Agogo utility timing. | Exceptional demonstrated narrow-workload upside, high mechanism confidence, now-high preparation confidence, and high but bounded API and packaging risk |
