...
|---:|---|---|---|---|---|
| 1 | [**FUTURE-ASSERT - Assertion-bearing conservative prefilter**](experiments/post-h11-future-optimization-portfolio.md) | reviewed; next experiment | Assertion-bearing databases currently disable every prefilter. The retained 1,000-pattern boundary fixture scans all starts and takes roughly 76 seconds over 8 MiB even though each consuming literal begins at the same start after a zero-width assertion. | Freeze a diagnostic-only candidate that extracts necessary literals past supported leading zero-width assertions, builds the existing false-positive-only candidate bitmap, and leaves assertion-aware semantic verification unchanged. | Very high narrow-workload opportunity, high evidence confidence, medium-low cost, medium semantic risk |
