...
> Generated from [`optimization-attempts.json`](optimization-attempts.json). Do not edit this file directly. Last reviewed: **2026-08-08**.
...
- **Mechanism:** Route dense H11 union candidates through compact partition masks while preserving the exact per-partition verifier and the unchanged sparse bitmap path.
...
- **Evidence:** [experiments/b2-h-0039-partition-routing.md](experiments/b2-h-0039-partition-routing.md)
...
- **Outcome:** `rejected`
- **Baseline:** `bacd5c46d934cd5526dcab78af88e375b2f13370`
- **Candidate:** `843ebafe947fd8749c0f8e843af4ce480b40c3b4`
- **Mechanism:** Compute eight exact five-unit FNV candidate hashes per AVX2 step while preserving conservative non-ASCII admission, scalar fallback, and the unchanged exact per-partition verifier.
...
- **Evidence:** [experiments/b2-h-0040-simd-candidate-discovery.md](experiments/b2-h-0040-simd-candidate-discovery.md)
...
- **Candidate:** `9bb4ad8f1b50db701ec1ecea66d9fdb73f2fae07`
- **Mechanism:** Conservatively classify assertion-free and assertion-bearing patterns, compile independent exact databases, and schedule both cohorts while preserving the union of exact events.
...
- **Evidence:** [experiments/h43-d1-r2-result.md](experiments/h43-d1-r2-result.md)

### H43-V1 - Shared-storage cohort roots and database views

- **Date:** 2026-08-06
- **Outcome:** `investigate`
- **Baseline:** `3d48cb754d89da1359582627f743af87a86b342a`
- **Candidate:** `4f08cd9e0fc7253c4333a27b61d860b7255c7e81`
- **Mechanism:** Compile assertion-free and assertion-bearing roots over one immutable pattern body store, with benchmark-only views and a prefilter-neutral carrier.
...
- **Evidence:** [experiments/h43-v1-result.md](experiments/h43-v1-result.md)

### H43-A1 - Isolated assertion-prefix backend over shared cohort storage

- **Date:** 2026-08-07
- **Outcome:** `investigate`
- **Baseline:** `4e83f540edda2cbcd2d29c25549f05e692d8fec5`
- **Candidate:** `8ad52d9600cb7de3c98ee345fd82fb06f9e7d429`
- **Mechanism:** Reconstruct the H24 assertion-prefix candidate scanner only inside H43-V1's benchmark-only shared assertion view, with one exact binary exposing forced generic and forced specialized modes and no production selector.
- **Measured result:** Exact 1 MiB and 2 MiB assertion targets improved 99.510% and 99.755% paired-geometrically, about 204x and 408x throughput. Events and activation were exact, allocation calls were unchanged, requested bytes increased by four, and RSS remained immaterial. The frozen analyzer rejected the artifact because one of thirteen inactive adversarial preparation pairs spiked enough to make its paired geometric preparation delta 20.366%.
- **Tradeoffs and caveats:** The rejecting cycle also raised unrelated input preparation from 0.772 ms to 10.709 ms while matcher preparation rose from 0.250 ms to 3.539 ms. The other twelve pairs clustered, specialized median preparation was 1.713% faster, both build entry points use the same compiler before storing one policy bit, and the ordinary no-feature scanner disassembly was unchanged. Post-hoc exclusion and rerunning until pass remain prohibited.
- **Decision:** The exact artifact is machine-rejected, unmerged, and excluded from the progress graph. The mechanism is investigate under G9-v3's high-discrepancy rule. H43-A1-P1 subsequently resolved the causal preparation question without changing this ruling and authorized both H43-A2 and the separately governed X1.1 ADR.
- **Evidence:** [experiments/h43-a1-result.md](experiments/h43-a1-result.md)

### H43-A1-P1 - Phase-local assertion-backend preparation discriminator

- **Date:** 2026-08-07
- **Outcome:** `investigate`
- **Baseline:** `8ad52d9600cb7de3c98ee345fd82fb06f9e7d429`
- **Candidate:** `8ad52d9600cb7de3c98ee345fd82fb06f9e7d429`
- **Mechanism:** Remove process startup, filesystem input, TSV parsing, regex registration, diagnostics, destruction, and serialization from the timed interval, then compare the generic and specialized shared-cohort build entry points in 31 AB and 31 BA pairs per fixture.
- **Measured result:** The clean Agogo window retained 372 builds. Paired-geometric effects were +0.0681% on the 1,000-pattern assertion target, -0.0274% on the 256-pattern adversarial assertion guard, and +0.0057% on the 1,000-pattern ordinary literal guard. Every 95% interval spanned zero and no directional count approached 27 of 31 in both orders.
- **Tradeoffs and caveats:** Every adjacent pair had identical allocation calls and requested bytes, and every compiled structural fact matched apart from the expected final specialization boolean. The result explains the original process-level spike but does not remove that cell or change H43-A1's machine rejection.
- **Decision:** Valid causal discriminator. Authorize H43-A2 and ADR-0009/X1.1. Keep H43-A1 rejected, unmerged, and excluded from the progress graph.
- **Evidence:** [experiments/h43-a1-p1-result.md](experiments/h43-a1-p1-result.md)

### H43-X1.5 - Formal explicit assertion matcher admission

- **Date:** 2026-08-08
- **Outcome:** `investigate`
- **Baseline:** `a5b86c2b3943aad3121e1f06944e39331f17a474`
- **Candidate:** `a5b86c2b3943aad3121e1f06944e39331f17a474`
- **Mechanism:** Run the separate feature-gated assertion-prefix matcher through calibrated G9-v3 default-safety, exactness, explicit-utility, boundary, fallback, allocation, RSS, and total-work lanes without adding automatic routing.
- **Measured result:** The complete 1,158-receipt assay passed both calibrations, all nine default guards, all four refusal probes, exact fallbacks, and every target scan and total-work gate. Assertion targets improved 99.569974% and 99.780726%, about 232.54x and 456.05x, with 15/15 positive cycles. The 2 MiB target preparation metric regressed 2.688833% with 13/15 adverse cycles.
- **Tradeoffs and caveats:** No default metric crossed 2%; the largest was a 1.616900% dense scan regression. Allocation probes were identical and no veto fired. The sole blocker is phase-suspicious because the same pattern database prepared 1.441057% faster in the 1 MiB cell and earlier construction-only evidence was neutral, but its directional burden prevents treating it as noise.
- **Decision:** Investigate. Do not merge or publish the exact artifact and do not add it to the progress graph. Authorize H43-X1.6 as a construction-only same-pattern discriminator; preserve this immutable formal result regardless of that successor.
- **Evidence:** [experiments/h43-x1-5-formal-admission-result.md](experiments/h43-x1-5-formal-admission-result.md)

### H43-X1.6 - Separate-type construction-only discriminator

- **Date:** 2026-08-08
- **Outcome:** `investigate`
- **Baseline:** `c47461695cbe7628f0f45ed61dd56bc2687ddd8d`
- **Candidate:** `c47461695cbe7628f0f45ed61dd56bc2687ddd8d`
- **Mechanism:** Compare generic and assertion-prefix-v1 construction in one feature-on executable using the exact same 1,000-pattern fixture, excluding corpus I/O, scanning, events, and serialization from the timed interval.
- **Measured result:** The clean formal window retained 60 primary, 120 same-type calibration, and six allocation invocations. Specialized complete preparation was 0.808000% slower with 14 of 15 adverse cycles; registration was 0.218342% slower and compilation was 1.103926% slower. Every effect remained below 2%, both calibrations passed, and allocation probes were exactly equal.
- **Tradeoffs and caveats:** The directional burden indicates a small construction cost rather than exact neutrality, but it cannot explain X1.5's 2.688833% process-level blocker under the unchanged rule. The discriminator narrows one causal uncertainty and is not a complete utility admission assay.
- **Decision:** Valid causal discriminator: the X1.5 construction blocker did not reproduce. Preserve X1.5 as investigate, do not merge automatically, and authorize explicit owner review without changing thresholds or the progress graph.
- **Evidence:** [experiments/h43-x1-6-construction-discriminator-result.md](experiments/h43-x1-6-construction-discriminator-result.md)

### H43-X1.7-R3 - Full explicit assertion matcher confirmation

- **Date:** 2026-08-08
- **Outcome:** `investigate`
- **Baseline:** `a5b86c2b3943aad3121e1f06944e39331f17a474`
- **Candidate:** `a5b86c2b3943aad3121e1f06944e39331f17a474`
- **Mechanism:** Run the single prospectively authorized complete confirmation using the exact X1.5 source, artifacts, fixtures, matrix, thresholds, and analyzer after hardening only host service restoration.
- **Measured result:** The clean five-hour confirmation retained 1,020 paired, 120 calibration, and 18 allocation receipts plus four functional probes. All nine default guards and every exactness, fallback, allocation, RSS, scan, and total-work gate passed. The 1 MiB and 2 MiB targets retained about 231.62x and 460.49x scan speedups, but the 2 MiB preparation metric regressed 5.792673% with 11 of 15 adverse cycles.
- **Tradeoffs and caveats:** The same-pattern 1 MiB preparation metric remained below the investigation boundary, and one-scan total work improved about 449.65x at 2 MiB. The vetoed sub-millisecond metric had roughly 6%-7% run-to-run variation and cycle effects ranging from an 8.165% gain to a 13.737% loss, but the predeclared estimator and directional burden remain binding.
- **Decision:** Machine-rejected by the unchanged greater-than-3% veto and not merged. Retain the complete R3 evidence permanently. Classify the mechanism investigate and authorize only the smaller H43-X1.8 corpus-conditioned causal discriminator; do not run another unchanged full assay.
- **Evidence:** [experiments/h43-x1-7-r3-confirmation-result.md](experiments/h43-x1-7-r3-confirmation-result.md)

### H43-X1.8 - Corpus-conditioned exact-source preparation discriminator

- **Date:** 2026-08-09
- **Outcome:** `investigate`
- **Baseline:** `a5b86c2b3943aad3121e1f06944e39331f17a474`
- **Candidate:** `8c27d2350125e3b0de6a8abdd65eff1947535b09`
- **Mechanism:** Materialize and retain the exact 2 MiB R3 fixture, then compare only registration, compilation, and complete construction over 15 balanced AB/BA cycles plus same-type calibrations. Repeat with a standalone diagnostic over the byte-exact rejected R3 rustmatch library tree.
- **Measured result:** The benchmark-layout run measured 0.717444% slower complete preparation; the exact-R3 library run measured 0.976161% slower preparation. Registration and compilation remained below 1% in the exact-source run. Both calibrations passed, all 360 receipts validated, and both guarded windows restored cleanly.
- **Tradeoffs and caveats:** The 13-of-15 and 14-of-15 adverse direction counts prove a small real construction premium rather than zero cost. Neither independent run reproduced the R3 5.792673% magnitude, and run-to-run variation fell below 1.6%. This evidence does not relabel or certify R3.
