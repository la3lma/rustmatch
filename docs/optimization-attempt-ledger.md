...
|---:|---|---|---|---|---|
| 1 | [**H43-X1.7 - Explicit assertion matcher owner review**](experiments/h43-x1-6-construction-discriminator-result.md) | available after neutral X1.6 discriminator | X1.5 retained exact 232.54x and 456.05x target scan speedups, complete default safety, and passing total-work economics, but formally remains investigate because one preparation metric regressed 2.688833%. X1.6 then isolated exact same-pattern construction and measured only a 0.808000% specialized cost, below 2%, so the suspected construction blocker did not reproduce. | Review the immutable X1.5 and X1.6 evidence together. Choose prospectively between one separately named full confirmation and an explicit owner-authorized merge exception. Neither option may relabel X1.5, omit the adverse receipt, alter thresholds, or present the experimental backend as default. | Exceptional explicit-capability utility with high evidence quality; low remaining causal uncertainty but medium governance risk because the formal X1.5 disposition is still investigate |
