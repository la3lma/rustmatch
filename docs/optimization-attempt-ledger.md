...
- **Decision:** The exact artifact is machine-rejected, unmerged, and excluded from the progress graph. The mechanism is investigate under G9-v3's high-discrepancy rule. Authorize only H43-A1-P1, a phase-local preparation discriminator; retain a two-key experimental backend only as a separately governed contingency.
...
|---:|---|---|---|---|---|
| 1 | [**H43-A1-P1 - Assertion-backend preparation discriminator**](experiments/h43-a1-result.md) | available after H43-A1 high-discrepancy review | H43-A1 reproduced exact 204x-408x paired-geometric target throughput with unchanged allocation count and stable RSS. Its sole machine blocker is a preparation aggregate dominated by one process that simultaneously spiked unrelated input preparation; static inspection finds identical build machinery apart from a final policy bit. | Freeze a phase-local single-process AB/BA microscope with at least 31 builds per order, parse fixtures outside the timed region, retain allocation receipts, and compare target, adversarial assertion, and ordinary literal construction. Do not alter or rerun away the rejected H43-A1 window. | Exceptional demonstrated assertion-heavy upside, high mechanism confidence, medium measurement confidence, and high selector/API risk |
