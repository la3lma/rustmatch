...
> Generated from [`optimization-attempts.json`](optimization-attempts.json). Do not edit this file directly. Last reviewed: **2026-08-04**.
...
| 8 | [**FUTURE-SIMD - SIMD shared candidate discovery**](experiments/b2-h-0040-simd-candidate-discovery.md) | evaluated as H40; rejected after bounded recovery | H40 proved 4.413x-4.641x exact kernel speedup and retained 22.937%-63.445% complete-scan gains. Both compiled candidates nevertheless crossed unchanged inactive-path guards; the recovery already restored scalar sampling, exact accepted build provenance, and hot-function mnemonic identity. | Do not retune H40 or tune function placement. Reopen only after a generally applicable stable production-layout boundary is independently justified, or reuse the kernel inside a materially different exact literal engine that must pass fresh unchanged guards. | Measured very high target opportunity and high mechanism confidence, but low near-term admission probability and medium-high portability/layout risk |
