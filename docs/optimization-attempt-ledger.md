...
- **Tradeoffs and caveats:** A four-key inline predecessor was rejected at L2 for doubling adversarial lookup time. Exact-one-key H19 improved memory but did not improve whole-matcher preparation, falsifying filter payload allocation as the preparation bottleneck.
- **Decision:** Diagnostically refuted before formal G9, not merged, and excluded from progress. The next discriminator must phase matcher construction rather than tune another storage threshold.
- **Evidence:** [https://github.com/la3lma/rmatch-performance-measurements/blob/209ad09073888300d6c1fb118c1b7b05bb7640f9/docs/lab-notebook/2026-07-28-b2-h-0019-inline-prefix-recovery.md](https://github.com/la3lma/rmatch-performance-measurements/blob/209ad09073888300d6c1fb118c1b7b05bb7640f9/docs/lab-notebook/2026-07-28-b2-h-0019-inline-prefix-recovery.md)
...
|---:|---|---|---|---|---|
| 1 | [**FUTURE-ASSERT - Phase-resolved assertion construction recovery**](https://github.com/la3lma/rmatch-performance-measurements/blob/209ad09073888300d6c1fb118c1b7b05bb7640f9/docs/lab-notebook/2026-07-28-b2-h-0019-inline-prefix-recovery.md) | H19 diagnostic refuted filter-allocation cause; phase audit required | H14-H19 repeatedly preserve roughly 185x-745x exact assertion scan gains. H16 and H17 recovered inactive scan blockers, while H18 and H19 reduced the target filter from 32,824 to 8,248 and then 64 bytes without recovering boundary-500 preparation. The remaining cost is outside the final filter payload. | Instrument repeated in-process H11, H18, and H19 matcher construction into parse/HIR metadata, assertion-prefix sidecar, NFA compile, prefilter compile, and final assembly phases with counted allocations. Authorize a new source candidate only if one phase reproduces the H11 penalty and offers a separable repair. | Very high narrow-workload reward, very high scan-mechanism confidence, medium diagnostic cost, and currently unknown construction-repair cost |
