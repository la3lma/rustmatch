...
> Generated from [`optimization-attempts.json`](optimization-attempts.json). Do not edit this file directly. Last reviewed: **2026-08-06**.
...
|---:|---|---|---|---|---|
| 1 | [**FUTURE-COHORT-VIEWS - Shared-storage cohort roots and database views**](experiments/h43-d1-r2-result.md) | design hypothesis after H43 carrier rejection | H43 proved exact mixed-cohort semantics and retained 57.17%-90.16% one-worker scan gains, but independently compiling each cohort caused 16.73%-23.19% preparation regressions and roughly 1,600 extra allocation calls. A single compiled database with lightweight cohort roots/views could preserve the separation signal without duplicating database-owned storage and construction. | Design a multi-root PatternDatabase representation and prove that ordinary single-root compilation and scan code remain byte-identical. Build a static allocation/state-count model before implementation, then run exact view/full-database event parity and preparation screens before any specialization. | Very high demonstrated narrow upside, medium-low representation confidence, high compiler and semantic-proof effort, and high inactive-layout risk |
