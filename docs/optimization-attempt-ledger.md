...
|---:|---|---|---|---|---|
| 1 | [**H43-X1.6 - Separate-type construction-only discriminator**](experiments/h43-x1-5-formal-admission-result.md) | authorized after X1.5 investigate | X1.5 proved exact 232.54x and 456.05x target scan speedups, complete default safety, and passing total-work economics, but the 2 MiB target preparation metric regressed 2.688833% with 13 of 15 adverse cycles. The same 1,000-pattern database prepared 1.441057% faster at 1 MiB and earlier phase-local evidence was neutral, so the sign-changing construction signal is both merge-blocking and plausibly separable. | Freeze a same-pattern construction-only crossover discriminator for the exact X2 separate types. Exclude corpus I/O and scanning, retain allocation and structure receipts, and require directional confidence. Repair a confirmed cost before repeating the complete assay; a neutral result may support owner review but cannot relabel X1.5. | Exceptional retained utility, narrow diagnostic effort, high causal value, and medium admission risk because the immutable formal blocker remains real until independently resolved |
