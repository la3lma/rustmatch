...
|---:|---|---|---|---|---|
| 1 | [**H43-V1 - Shared-storage cohort roots and database views**](experiments/b2-h-0043-cohort-aware-matcher-plan.md#h43-v1-prove-shared-storage-cohort-roots-and-database-views) | active benchmark-only feasibility lane | H43 proved exact mixed-cohort semantics and retained 57.17%-90.16% one-worker scan gains, but independently compiling each cohort caused 16.73%-23.19% preparation regressions and roughly 1,600 extra allocation calls. V1 now compiles both cohort roots over one body store; the static invariant is one extra state with no extra compiled edges, predicates, terminals, or pattern IDs. | Complete exact view/full-database parity and repository CI, verify ordinary emitted text, then run the predeclared focused preparation/allocation screen before any specialization. | Very high demonstrated narrow upside, medium representation confidence after the structural proof, high semantic and inactive-layout scrutiny |
