...
> Generated from [`optimization-attempts.json`](optimization-attempts.json). Do not edit this file directly. Last reviewed: **2026-07-27**.
...
|---:|---|---|---|---|---|
| 1 | [**FUTURE-ASSERT - Assertion-dispatch isolation and prefilter recovery**](experiments/b2-h-0013-assertion-prefilter-recovery-result.md) | H13 investigated; narrower H14 recovery requires fresh review | H13 preserved exact 216x-727x assertion gains and removed H12's assertion-plan monomorphizations, but Wuthering still regressed 5.791% and repeated preparation regressed 5.862%-13.359%. Static evidence shows the ordinary scanner still changed shape, while prefix proof retained 32 units for a filter that consumes at most five. | Freeze a five-unit assertion prefix representation plus a cold or separately compiled assertion entry. Require ordinary scanner and caller shape parity before reusing H13's focused target, preparation, Wuthering, and H11-shared gates unchanged. | Very high narrow-workload opportunity, very high algorithm confidence, low-medium recovery cost, and medium compiler-layout risk |
